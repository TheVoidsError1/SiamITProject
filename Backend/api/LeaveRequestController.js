  ...
  module.exports = (AppDataSource) => {
    ...
    // POST /api/leave-request (for regular users)
    ...
    router.get('/my', async (req, res) => {
     ...
     // POST /api/admin/leave-request (for admin to submit leave for employees)
     router.post('/admin', leaveAttachmentsUpload.array('attachments', 10), async (req, res) => {
       try {
         ...
         if (adminRole !== 'admin' && adminRole !== 'superadmin') {
           return sendUnauthorized(res, 'Only admin and superadmin can submit leave for employees');
         }

                 const {
          repid, // ID ของพนักงานที่ต้องการกรอกใบลาให้
          leaveType, 
          startDate, 
          endDate,
          startTime, 
         ...
         const processRepo = AppDataSource.getRepository('ProcessCheck');
         const employee = await processRepo.findOneBy({ Repid: repid });
         if (!employee) {
           return sendNotFound(res, 'Employee not found');
         }

         // ดึงตำแหน่งของพนักงาน
         let employeeType = null;
         if (employee.Role === 'admin') {
           const adminRepo = AppDataSource.getRepository('Admin');
           const admin = await adminRepo.findOneBy({ id: repid });
           employeeType = admin ? admin.position : null;
         } else if (employee.Role === 'superadmin') {
           const superadminRepo = AppDataSource.getRepository('SuperAdmin');
           const superadmin = await superadminRepo.findOneBy({ id: repid });
           employeeType = superadmin ? superadmin.position : null;
         } else {
           const userRepo = AppDataSource.getRepository('User');
           const user = await userRepo.findOneBy({ id: repid });
           employeeType = user ? user.position : null;
         }
         ...
         // ตรวจสอบ quota (ยกเว้น Emergency Leave)
         if (leaveTypeEntity && leaveTypeEntity.leave_type_en !== 'Emergency') {
           const leaveQuotaRepo = AppDataSource.getRepository('LeaveQuota');
           const quotaRow = leaveTypeEntity ? await leaveQuotaRepo.findOne({ where: { positionId: employeeType, leaveTypeId: leaveTypeEntity.id } }) : null;
           
           if (!quotaRow) {
             return sendValidationError(res, lang === 'th' ? 'ไม่พบโควต้าการลาสำหรับตำแหน่งนี้' : 'No leave quota found for this position');
           }

           // ตรวจสอบโควต้า
           const leaveUsedRepo = AppDataSource.getRepository('LeaveUsed');
           const usedLeave = await leaveUsedRepo.findOne({ where: { user_id: repid, leave_type_id: leaveTypeEntity.id } });
           const usedDays = usedLeave ? usedLeave.days : 0;
           
           if (usedDays >= quotaRow.quota) {
             return sendValidationError(res, lang === 'th' ? 'โควต้าการลาหมดแล้ว' : 'Leave quota exceeded');
           }
         }

         const attachmentsArr = req.files ? req.files.map(f => f.filename) : [];
         
         // ตรวจสอบว่าเป็นการลาย้อนหลังหรือไม่
         let backdated = 0;
         if (startDate) {
           const today = new Date();
           today.setHours(0, 0, 0, 0);
           ...
           if (leaveStart && leaveStart < today) {
             backdated = 1;
                 ...
                 // Calculate duration and durationHours based on durationType
        let days = null;
        ...
        if (durationType === 'hour' && startTime && endTime) {
          ...
          durationHours = diff;
        } else if (durationType === 'day' && startDate && endDate) {
          // Calculate days for daily leave
          const start = new Date(startDate);
          const end = new Date(endDate);
          days = Math.abs((end - start) / (1000*60*60*24)) + 1;
        }

        // Determine final status based on approvalStatus
        ...
        const leaveData = {
          Repid: repid, // ใช้ ID ของพนักงานที่เลือก
          employeeType,
          leaveType,
          startDate,
          endDate,
          ...
          approvedTime,
          rejectedTime,
          rejectedReason: approvalNote || null, // Store approval note in rejectedReason field
          backdated,
        };
        ...
        // If approved, update LeaveUsed table
        if (finalStatus === 'approved' && leaveTypeEntity) {
          try {
            const leaveUsedRepo = AppDataSource.getRepository('LeaveUsed');
            let leaveUsed = await leaveUsedRepo.findOne({ 
              where: { user_id: repid, leave_type_id: leaveTypeEntity.id } 
            });
            
            if (!leaveUsed) {
              leaveUsed = leaveUsedRepo.create({
                user_id: repid,
                leave_type_id: leaveTypeEntity.id,
                days: 0,
                hour: 0
            ...
            // Update used days/hours
            if (durationType === 'hour' && durationHours) {
              leaveUsed.hour = (leaveUsed.hour || 0) + durationHours;
            } else if (durationType === 'day' && days) {
              leaveUsed.days = (leaveUsed.days || 0) + days;
            }
            
            await leaveUsedRepo.save(leaveUsed);
          } catch (error) {
            console.error('Error updating LeaveUsed:', error);
            // Don't fail the request if LeaveUsed update fails
          }
        }
        ...
        if (io) {
          io.emit('newLeaveRequest', {
            ...
            employeeId: repid,
            message: lang === 'th' ? 'มีคำขอลาใหม่จากพนักงาน' : 'New leave request from employee'
          });
        }

        sendSuccess(res, savedLeave, lang === 'th' ? 'ส่งคำขอลาสำเร็จ' : 'Leave request submitted successfully');

       } catch (err) {
         console.error('Error in admin leave request:', err);
